...
Looks like there are some NA's in continent. Let's find which countries
```{r}
data %>% 
...
# a few more (non-intuitive) programming things that are super useful

Making a vector in R

```{r}
vector_of_numbers <- c(1,3,5,7,92,4,5)
vector_of_letters <- c("a","f","t")
a_quick_way <- 1:10
a_more_flexible_quick_way <- seq(from=10,to=20,by=2)
```

### "Is it in?" 

Store this operation in your memory banks, and why this is useful will become clear later.

```{r}
14 %in% a_more_flexible_quick_way
15 %in% a_more_flexible_quick_way
c(14,16,17) %in% a_more_flexible_quick_way
```

# Advanced data visualisation with `ggplot2`
